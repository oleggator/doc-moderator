...
def find_words(target_words_file_path, article_path, output_path, ld):
    ...
    if extension == '.docx':
        ...
        for paragraph in document.paragraphs:
            for word in fokin_split(paragraph.text):
                article_words.add(word)
    else:
        with open(article_path, 'r') as file:
            article_words = set(fokin_split(file.read()))

    with open(target_words_file_path, 'r') as file:
        target_words = set(fokin_split(file.read()))
...
def fokin_split(test):
    pun = '!"#$%&\'()*+,–./:;<=>?@[\\]^_`{|}~”'
    translator = str.maketrans(pun, ' ' * len(pun)) #map punctuation to space
    return test.lower().translate(translator).split()

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='find words in document')
    parser.add_argument('-d', required=True, action='store', dest='target_words',
                        help='dictionary (txt)')
    parser.add_argument('-a', required=True, action='store', dest='article',
                        help='article (txt or docx)')
    parser.add_argument('-o', required=True, action='store', dest='output',
                        help='found words output file')
    parser.add_argument('-l', required=True, action='store', dest='distance', type=int,
